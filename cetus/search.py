...
def process_query(index, query, start, end, raw, metadata):
    ...
    if index == 'DNS':
        query_string = f'host.raw:{query} AND collection_timestamp:[{start} TO {end}]&index=dns'
    else:
    ...
    with console.status(f'[bold green]Searching Cetus {index} index...'):
        results = requests.get(f'{api_url}{query_string}', headers={'Authorization': f'Token {api_key}'})
        try:
...
def dns(
    ...
    end: datetime = typer.Option(str(now), help="End of search range. If omitted, end == now"),
    raw: bool = typer.Option(False, "--raw", help="Print raw output instead of pretty printed"),
    metadata: bool = typer.Option(False, "--metadata", help="Include metadata in output")
):
    """
